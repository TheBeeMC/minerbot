...
async def on_message(message):
    if message.content.startswith('.bbbg'):
            bot=discord.Embed(title= Welcome to the Bevydia Discord!, color=0x0000ff)
            bot.add_field(name=__*Here are all the rules and information for our server!* __, value=Check out our Twitter: https://twitter.com/Bevydia. , inline=True)
            bot.add_field(name=, value=Please take note of the channel topics at the top to find the correct , inline=False)
            bot.add_field(name=, value=channel., inline=False)
            bot.add_field(name=, value=Invite others using https://discord.gg/Nqq4fcr. , inline=False)
